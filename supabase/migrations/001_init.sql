...
-- Supabase migrations for Secure Media Vault (extended per spec)
...
-- example policies (refine for production)
...
for all
to authenticated
using (exists(select 1 from public.asset a where a.id = asset_id and a.owner_id = auth.uid()))
with check (exists(select 1 from public.asset a where a.id = asset_id and a.owner_id = auth.uid()));
